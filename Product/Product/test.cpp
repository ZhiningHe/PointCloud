...
int main()
{
	const Mat srcImage = imread("H:\Harris.jpg");
	if (!srcImage.data)
	{
	...
	harrisResponse(GaussianXX, GaussianYY, GaussianXY, HarrisRespond, 0.04);	//响应函数
...
{
	//断言assert
	CV_Assert(srcImage.channels() == 1); // 只处理单通道
	int kerR = size / 2;
	//拷贝一个灰度图，保证大小一致
	dst = srcImage.clone();
	double* arr;
		...
		for (int j = r; j < ResultImage.cols - r; j++)
		{
				...
				resultData.at<float>(i, j) > resultData.at<float>(i + 1, j + 1))
			{
				...
				if ((int)resultData.at<float>(i, j) >Thr)
				{
					...
					//在结果图上标注提取角点
					circle(ResultImage, Point(j, i), 5, Scalar(0, 255, 0), 1, 8, 0);
				}
			}
